...
{
      NSData* publicKey = [SecKey stripPublicKeyHeader:[NSData dataFromBase64String:keyString]];
      
      return [SecKey addPublicKey:keyName keyBits:publicKey];
...
+ (SecKeyRef)addPublicKey:(NSString *)keyName keyBits:(NSData *)publicKey
{
      ...
      if (persistPeer)
      {
            peerKeyRef = [SecKey getKeyRefWithPersistentKeyRef:persistPeer];
      }
      else
      {
...
+ (BOOL)verifySignature:(NSData *)plainText secKeyRef:(SecKeyRef)publicKey signature:(NSData *)sig
{
      ...
      sanityCheck = SecKeyRawVerify(    publicKey,
                                    kTypeOfSigPadding,
                                    (const uint8_t *)[[SecKey getHashBytes:plainText] bytes],
                                    kChosenDigestLength,
...
+ (NSData*)encrypt:(NSData*)plainText publicKey:(NSData*)key
{
      ...
      publicKeyRef = [SecKey addPublicKey:keyName keyBits:[SecKey stripPublicKeyHeader:key]];
      ...
      [SecKey removePublicKey:keyName];
